    ...
    def main(self):
        ...
        for line in file_assemble:
            ...
            if (len(line) > 3):  # error for more operands
                ...
                file_error.write(str(self.line_number))
                file_error.write('\n')
            ...
            if self.is_declarative_statement(line):  # make function for handling declarations here
                ...
                if not self.symbol_already_exists(line[0]):
                    symbol_table[line[0]] = {'type': 'variable', 'used': False, 'defined': True,
                                             'address': self.location_counter, 'value': line[2]}
                else:
            ...
            if self.has_a_label(line):
                label = self.extract_label(
                    line)  # send it to symbol table later and check if it already exists for error reporting
                line = line[1:]
                if not self.symbol_already_exists(label):
                    symbol_table[label] = {'type': 'label', 'used': False, 'defined': True,
                                           'address': self.location_counter, 'value': 0}
                else:
            ...
            if self.valid_opcode(opcode):  # requires case for invalid for now
                ...
                if a[1] != 0:  # nothing done for literal yet
                    if (len(line) != 2):
                        if(len(line)<2):
                            file_error.write("Not enough operands supplied in line " + str(self.line_number) + '\n')
                        else:
                            file_error.write("Extra operands supplied in line " + str(self.line_number) + '\n')
                        # self.fatal_error=True
                        self.error_found = True
                        continue
                    ...
                    if a[1] == 2:
                        if not self.symbol_already_exists(operand):
                            symbol_table[operand] = {'type': 'label', 'used': True, 'defined': False, 'address': None,
                                                     'value': 0}
                        else:
                        ...
                        if not self.symbol_already_exists(operand):
                            symbol_table[operand] = {'type': 'variable', 'used': True, 'defined': False,
...
def pass_one():
    ...
    if len(file_error.readlines()) != 0:
        file_error.close()
        file_error = open("error.txt", 'r')
        print("ERRORS")
        while (True):
            t = file_error.readline()
            if t == '':
                break
            else:
                print("1. " + t)
